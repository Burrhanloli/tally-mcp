...
The server implements three main tools for Tally integration:

1. **get_day_book(date)**: Retrieves daybook entries for a specific date
2. **get_ledger_vouchers(ledger_name, from_date, to_date)**: Gets all vouchers for a specific ledger within date range
3. **get_all_ledgers()**: Lists all available ledger accounts
