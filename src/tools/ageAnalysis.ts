  ...
  server.registerToolWithHandler(
    ...
    "Retrieves aging analysis for a specific ledger (receivables/payables analysis).",
    {
      ...
      properties: {
        ledger_name: {
          type: "string",
          description: "Name of the ledger for aging analysis.",
        },
        date: {
          type: "string",
          description: "Date for analysis in DD-MM-YYYY format.",
        },
      },
      required: ["ledger_name", "date"],
    },
    async (args: { ledger_name: string; date: string }) => {
      const { ledger_name, date } = args;
      ...
      try {
        ...
        if (!bills || bills.length === 0) {
          return `No bills/transactions found for '${ledger_name}' as of ${date}.`;
        }

        let ageAnalysis = `Age Analysis - ${ledger_name} as of ${date}:\n\n`;
