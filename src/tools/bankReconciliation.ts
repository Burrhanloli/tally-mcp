...
const bankReconciliationSchema = z.object({
	bankLedger: z.string().describe("Name of the bank ledger to reconcile."),
	date: z.string().describe("Date for reconciliation in DD-MM-YYYY format."),
});
...
export function registerBankReconciliationTool(
	...
	server.registerToolWithHandler(
		...
		async (args: { bankLedger: string; date: string }) => {
			const { bankLedger, date } = args;
        ...
        <BODY>
			...
			try {
				...
				if (!vouchers || vouchers.length === 0) {
					return `No bank transactions found for '${bankLedger}' as of ${date}.`;
				}

				let reconciliation = `Bank Reconciliation - ${bankLedger} as of ${date}:\n\n`;
